...
-- S'il y a un changement dans la table d'origine, la vue est corrigée dynamiquement car elle enregistre la requête SELECT qui pointe vers la table d'origine.
-- Inversement, s'il y a un changement dans la table virtuelle, il s'impacte dans la table d'origine. 

-- Il y a intérêt à faire des vues en termes de gain de ressources, ou quand il y a des requêtes complexes à éxécuter.

SHOW TABLES; -- Cette vue est visible dans la liste des tables avec cette instruction

DROP VIEW vue_homme; -- Supprimer cette vue
...
SELECT prenom FROM temp; -- Affiche les prénoms de la table temp
