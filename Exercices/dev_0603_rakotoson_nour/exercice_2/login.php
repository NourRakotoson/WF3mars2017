...
if (!empty($_POST)) {
    ...
    if (empty($_POST['email'])) {
        $contenu .= '<div class="bg-danger">L\' email est requis</div>';
    }
    ...
    if (empty($contenu)) {
        ...
        $password = md5($_POST['password']); // on crypte le password pour le comparer à celui de la base

        $resultat = executeRequete("SELECT * FROM membre WHERE email = :email AND password = :password", array(':email' => $_POST['email'], ':password' => $password));

        if ($resultat->rowCount() !=0) { // s'il y a un enregistrement dans le résultat, c'est que le email et le mdp correspondent
            $membre = $resultat->fetch(PDO::FETCH_ASSOC); // pas de while car le email est unique
            echo '<pre>'; print_r($membre); echo '</pre>';
			$contenu .=  '<div class="bg-success">Vous êtes connecté</div>';
