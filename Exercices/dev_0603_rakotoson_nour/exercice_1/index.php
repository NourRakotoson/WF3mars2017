...
if(!empty($_POST)){
	...
	if(strlen($post['firstname'] < 3)){
		$errors = 'Le prénom doit comporter au moins 3 caractères<br>';		
	}
	if(strlen($post['lastname'] < 3)){
		$errors .= 'Le nom doit comporter au moins 3 caractères<br>';
	}
	if(!filter_var($post['email'], FILTER_VALIDATE_EMAIL)){
		$errors .= 'L\'adresse email est invalide<br>';
	}
	if(empty($post['birthdate'])){
		$errors .= 'La date de naissance doit être complétée<br>';
	}
	if(empty($post['city'] < 3)){
		$errors .= 'La ville ne peut être vide<br>';
	}
	if(count($errors) == 0){ 
