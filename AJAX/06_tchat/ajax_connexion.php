...
$erreur = false; // variable de contrôle en fin de script. Si sa valeur est passée à true alors il y a une erreur (exemple le pseudo indisponible)

// extract($_POST); créé des variables que l'on ne nomme pas, peut provoquer des erreurs
...
if($mode == "connexion") {
    ...
    // requête testant si le pseudo est déjà présent dans la BDD
    ...
    elseif ($resultat->rowCount() > 0 && $_SERVER['REMOTE_ADDR'] == $membre['ip'])
    {
        ...
        // dans ce cas là, on met à jour uniquement sa date de connexion
        $time = time();
    ...
    if(!$erreur) // if($erreur == false) // s'il n'y a pas d'erreur durant les traitements précédents
    {
        ...
        $f =    fopen("prenom.txt", "a"); // on ouvre un fichier prenom.txt sinon on le créé
