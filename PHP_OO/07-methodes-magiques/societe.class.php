...
class Societe
{
    ...
    public function __get($nom){
        echo '<p style="color: white; background: red; padding: 5px"> Désolé , mais la propriété ' . $nom . ' n\'existe pas dans cette classe !</p>';
    }  

    /*
    __call() = quand j'appelle une méthode qui n'existe pas.
    __callstatic() = quand j'appelle une méthode statique qui n'existe pas.
    __isset() = quand on fait une condition isset ou empty sur une propriété de mon objet
    __destruct() = s'exécute quand le script est terminé (pratique pour fermer une connexion à la BDD par exemple). 
    __toString() = se lance quand on essaie de faire un echo sur un objet

    __wake(), __sleep(), __invoke(), __clone()...   
    */
}
