...
// La méthode fetch() avec le paramètre PDO//FETCH_ASSOC permet de transformer l'objet $result en un ARRAY associatif exploitable indexé avec le nom des champs de la requête
...
$result = $pdo->query("SELECT * FROM employes WHERE prenom = 'daniel'");
$employe = $result->fetch(PDO::FETCH_NUM); // pour un array indexé numériquement
echo '<pre>'; print_r($employe); echo '</pre>'; 
...
echo '<pre>'; print_r($employe); echo '</pre>'; 
echo $employe->prenom . '<br>'; // affiche la propriété prenom de l'objet $employe

// Attention : il faut choisir l'un des fetch que vous voulez exécuter sur un jeu de résultats, vous ne pouvez pas faire plusieurs fetch sur le même résultat n'en contenant qu'un seul. En effet, cette méthode déplace un curseur de lecture sur le résultat suivant contenu dans le jeu résultat : ainsi quand il n'y en a qu'un seul, on sort du jeu. 
...
while ($contenu = $resultat->fetch(PDO::FETCH_ASSOC)) { // fetch retourne la ligne suivant du jeu de résultats en array associatif. La boucle while permet de faire avancer le curseur dans le jeu de résultats et s'arrête quand il est à la fin des résultats. 
...
// Quand vous faîtes une requête qui ne sort qu'un seul résultat : pas de boucle while, mais un fetch seul. 
...
$donnees = $resultat->fetchAll(PDO::FETCH_ASSOC); // retourne toutes les lignes de résultats dans un tableau multidimensionnel sans faire de boucle (c'est automatique) : vous avez un array associatif à chaque indice numérique. Marche aussi avec FETCH_NUM

//echo '<pre>'; print_r($donnees); echo '</pre>';

// Pour lire le contenu d'un array multidimmensionnel, nous faisons des boucles foreach imbriquées:
echo '<strong>Double boucle foreach</strong>';

foreach ($donnees as $contenu) { // $contenu est sous array associatif
...
echo '<ul>'; 
    while ($listedatabases = $resultat->fetch(PDO::FETCH_ASSOC)){
       echo '<li>' . $listedatabases["Database"] . '</li>';
    }
echo '</ul>'; 
...
echo '<table border = 1>';
    ...
    while($ligne = $resultat->fetch(PDO::FETCH_ASSOC)) {
        echo '<tr>';
            foreach($ligne as $information){ // après le as forcément des values mais on peut l'appeler comme on veut ici : $information
...
$resultat->bindParam(':nom', $nom, PDO::PARAM_STR); // je lie le marqueur :nom à la variable $nom. Si on change le contenu de la variable, la valeur du marqueur chargera automtiquement si on fait plusieurs execute

// On exécute la requête : 
$resultat->execute();

$donnees = $resultat->fetch(PDO::FETCH_ASSOC); // $donnees est un array associatif
echo implode($donnees, ' - '); // implode transform l'array en string
...
/*
    ...
    Par ailleurs, les requêtes préparées sont souvent utilisées pour assainir les données en forçant le type de valeurs communiquées aux marqueurs. 
*/
...
//********************************
echo '<h1>08. Requête préparée : prepare() + bindValue() + execute()</h1>';
...
// On lie le marqueur :nom à une valeur
$resultat->bindValue(':nom', $nom, PDO::PARAM_STR); // binValue() reçoit une variable ou un string. Le marqueur pointe uniquement vers la valeur : si celle-ci change, il faudra refaire un bindValue() pour prendre en compte cette nouvelle valeur lors d'un prochain execute()
...
$resultat->execute(); // on obtient un objet issu de la classe prédéfinie PDOStatement (=résultat de requête)
...
class Employes {
    ...
    public $salaire; // on déclare autant de propriétés qu'il y a de champs dans la table employée. L'orthographe des propriétés DOIT être identique à celle des champs. 
}
...
foreach ($donnees as $unemploye) {
    echo $unemploye->prenom . '<br>';
}
...
$resultat->execute(array('durand', 'damien')); // 'durand' va remplacer le "?" et 'damien' le second
...
echo implode($donnees, ' - '); // notez que nous faisons implode ici pour aller plus vite et éviter de faire un affichage dans une boucle... (on utilise implode ici pour que le code soit plus digeste)
...
echo '<pre>'; echo print_r($resultat->errorInfo()); echo '<pre>'; //errorInfo() est une méthode qui appartient à la classe PDOStatement et qui fournit des infos sur l'erreur SQL éventuellement produite. On trouve le message de l'erreur à l'indice[2] de l'array retourné par cette méthode. 
