...
if (internauteEstConnecte()) { // si l'internaute est déjà connecté, il n'y a rien à faire ici, on le redirige donc vers son profil
...
if (!empty($_POST)) {
    ...
    if (empty($contenu)) {
        $mdp = md5($_POST['mdp']); // on crypte le mdp pour le comparer à celui de la base

        $resultat = executeRequete("SELECT * FROM membre WHERE pseudo = :pseudo AND mdp = :mdp", array(':pseudo' => $_POST['pseudo'], ':mdp' => $mdp));

        if ($resultat->rowCount() !=0) { // s'il y a un enregistrement dans le résultat, c'est que le pseudo et le mdp correspondent
            $membre = $resultat->fetch(PDO::FETCH_ASSOC); // pas de while car le pseudo est unique
